describe('New Item', () => {
  ...
  let testData = [
    { fixture: 'itemMetadataArticleRequiredFields', genre: 'Article', itemData: undefined as any },
    { fixture: 'itemMetadataConferencePaperRequiredFields', genre: 'Conference Paper', itemData: undefined as any }
  ]

  before(() => {
    testData.forEach(({fixture, genre}, index) => {
      cy.fixture(fixture).then((data) => {
        testData[index].itemData = data
      })
    })
  })
  ...
  testData.forEach((testCase) => {
    it(`Create a new item via the edit page with genre ${testCase.genre} and only required fields`, () => {
      const itemData = testCase.itemData
